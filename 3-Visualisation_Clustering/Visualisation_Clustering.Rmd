...
# Loading data
```{r}
myeloid_cells <- readRDS("Myeloid_cells_Part2.rds")
```
...
dittoBarPlot(myeloid_cells, "seurat_clusters", group.by = "orig.ident", color.panel = colors, main ="", var.labels.reorder = var_order, var.labels.rename = labels, x.labels = c("Ly6G Macs", "Mock", "PR8", "Mo-IM"), x.labels.rotate = F, xlab = "Sample")+ plot_annotation(
...
```{r}
cell_myeloid_Pr8 <- subset(myeloid_cells, orig.ident %in% "cell_myeloidPr8")
...
cell_myeloid_IMcells <- subset(myeloid_cells, orig.ident %in% "IM_cells")
...
# Visualisation of Motro Cells
```{r, message=FALSE, warning=FALSE, results= "hide"}
cell_myeloid_Ly6GMacs <- subset(myeloid_cells, orig.ident %in% "cell_Motro_Pr8")

DimPlot(cell_myeloid_Ly6GMacs, reduction = "umap", cols = colors, group.by = "CellType") + plot_annotation(
  title = 'Myeloid Cells - Ly6G Macs',
  theme = theme(plot.title = element_text(size = 16, hjust = 0.5)))

#ggsave("UMAP_Ly6GMacs.pdf", height = 12 , width = 19, units = "cm")
```
