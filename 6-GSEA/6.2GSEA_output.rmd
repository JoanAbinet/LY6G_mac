...
## KEGG results
```{r}
GSEA_KEGG_res <- read.table(paste0(dir,"my_analysis.Gsea.1675776471604_KEGG/gsea_report_for_Motro_1675776471604.tsv"), header = T, sep = "\t")

GSEA_KEGG_res<- GSEA_KEGG_res[order(GSEA_KEGG_res$NES, decreasing = T),]

#GSEA_KEGG_res <- modify_GSEA_data(GSEA_KEGG_res)

GSEA_KEGG_res <- GSEA_KEGG_res[c(4,8,9,10,11,12,26,37,38,45,47),]

GSEA_KEGG_res$NAME <- fct_rev(factor(GSEA_KEGG_res$NAME, levels = GSEA_KEGG_res$NAME))
```

```{r fig.width=10, fig.height=5}
ggplot(GSEA_KEGG_res, aes(x = NES,y =NAME, size = SIZE, color = FDR.q.val)) + geom_point() + ggtitle("GSEA - KEGG") + scale_color_gradient(low = "red", high = "blue") + theme_bw()+ scale_size(range = c(3,10))
